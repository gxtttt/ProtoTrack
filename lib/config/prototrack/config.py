...
# PRN (Prototype Reconstruction Network) BACKBONE
cfg.MODEL.BACKBONE.PRN_LOC = []  # PRN模块插入位置
cfg.MODEL.BACKBONE.PRN_NUM_PROTOTYPES = 128  # 原型数量

# CAF (Cross-modal Attention Fusion) BACKBONE
cfg.MODEL.BACKBONE.CAF_ENABLED = False  # 启用CAF跨模态融合模块
cfg.MODEL.BACKBONE.CAF_NUM_HEADS = 8    # CAF模块的注意力头数
cfg.MODEL.BACKBONE.CAF_FEATURE_DIM = 768  # CAF模块的特征维度

#CA (Cross-modal Attention Fusion)BACKBONE，简单交叉注意力模块
cfg.MODEL.BACKBONE.CA_LOC = []  # CA模块插入位置
...
# PRN (Prototype Reconstruction Network)
cfg.TRAIN.PRN_DROP_PATH = []  # drop_path rate for PRN layers

# CAF (Cross-modal Attention Fusion)
cfg.TRAIN.CAF_DROP_PATH = []  # drop_path rate for CAF layers

# CA (Cross-modal Attention) 
cfg.TRAIN.CA_DROP_PATH = []  # drop_path rate for CA layers
...
cfg.TEST.EPOCH = 500
cfg.TEST.MAX_TEMPLATES = 1
cfg.TEST.UPDATE_INTERVAL = 25
cfg.TEST.CONF_THRESHOLD = 0.7
